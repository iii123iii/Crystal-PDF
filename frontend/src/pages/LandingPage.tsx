    ...
    <div className="min-h-screen bg-[#050e18] text-slate-200 relative">
      ...
      <nav className="sticky top-0 z-50 backdrop-blur-xl border-b border-white/[.04]"
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between px-4 sm:px-6 h-14 sm:h-16">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            <Link to="/login"
              className="text-xs sm:text-sm text-slate-400 hover:text-white px-3 sm:px-4 py-2 transition-colors">
              Sign in
            </Link>
            <Link to="/register"
      ...
      <section className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6
        ...
        <div className="max-w-2xl">
          <h1 className="font-display font-semibold tracking-tight leading-[1.1]
                         text-[clamp(1.8rem,7vw,4.8rem)] sm:text-[clamp(2.2rem,5.5vw,4.8rem)] text-white anim-reveal">
          ...
          <p className="mt-5 sm:mt-7 text-sm sm:text-[clamp(1rem,1.8vw,1.2rem)] leading-relaxed
          ...
          <div className="mt-8 sm:mt-10 flex flex-col sm:flex-row flex-wrap gap-2.5 sm:gap-3 anim-reveal anim-d3">
            <Link to="/register"
              className="inline-flex items-center justify-center gap-2 px-5 sm:px-6 py-3 sm:py-3.5 rounded-xl text-sm sm:text-[15px] font-medium
                         ...
                         hover:-translate-y-px active:translate-y-0 transition-all w-full sm:w-auto">
              Start for free <ArrowRight size={14} strokeWidth={2.2} />
            </Link>
            <Link to="/login"
              className="inline-flex items-center justify-center gap-2 px-5 sm:px-6 py-3 sm:py-3.5 rounded-xl text-sm sm:text-[15px] font-medium
                         text-slate-300 border border-white/10 hover:border-white/20
                         hover:bg-white/[.03] transition-all w-full sm:w-auto">
      ...
      <section className="relative z-10 dot-grid">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-20 sm:py-24 md:py-32">
          ...
          <div className="max-w-lg mb-10 sm:mb-14">
            <p className="text-[10px] sm:text-xs font-medium tracking-[.15em] uppercase text-brand-400 mb-3 sm:mb-4">
              Toolkit
            </p>
            <h2 className="font-display text-[clamp(1.7rem,6vw,3.5rem)] sm:text-[clamp(2rem,4.5vw,3.5rem)] font-semibold
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3">
            {tools.map((t) => (
              <div key={t.name} className="tool-card rounded-2xl p-4 sm:p-5 cursor-default group">
                ...
                <p className="text-sm sm:text-[15px] font-semibold text-white tracking-tight mb-1">
                  {t.name}
                </p>
                ...
                <div className="flex items-start gap-3 sm:gap-5">
                  ...
                  <div className="flex flex-col items-center">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center
                      ...
                      <s.icon size={18} className="text-blue-400" strokeWidth={1.6} />
                    </div>
                  </div>
      ...
      <section className="relative z-10 border-t border-white/[.04]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-20 sm:py-28 md:py-36 text-center">
          ...
          <h2 className="font-display text-[clamp(1.7rem,6.5vw,4.5rem)] sm:text-[clamp(2.4rem,5.5vw,4.5rem)] font-semibold
                         tracking-tight leading-[.95] text-white mb-8 sm:mb-10 relative">
            Start working with<br />
            your PDFs today.
          </h2>
          <div className="relative flex flex-col sm:flex-row flex-wrap justify-center items-center gap-2.5 sm:gap-3">
            <Link to="/register"
              className="inline-flex items-center justify-center gap-2 px-5 sm:px-7 py-3 sm:py-3.5 rounded-xl text-sm sm:text-[15px] font-medium
                         text-white bg-gradient-to-b from-brand-500 to-brand-600
                         shadow-[0_2px_24px_rgba(45,98,255,.35)]
                         hover:shadow-[0_4px_32px_rgba(45,98,255,.5)]
                         hover:-translate-y-px transition-all w-full sm:w-auto">
              Create free account <ArrowRight size={14} strokeWidth={2.2} />
            </Link>
            <Link to="/login"
              className="inline-flex items-center justify-center gap-2 px-5 sm:px-7 py-3 sm:py-3.5 rounded-xl text-sm sm:text-[15px] font-medium
                         text-slate-400 hover:text-slate-200 transition-colors w-full sm:w-auto">
              or sign in
            </Link>
          </div>
        </div>
