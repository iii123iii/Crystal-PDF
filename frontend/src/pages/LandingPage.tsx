...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-[#050e18] text-slate-200 relative overflow-x-hidden w-full">
      {/* ── CSS ─────────────────────────────────────────────── */}
      <style>{`
      ...
      <section className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6
        ...
        <div className="max-w-2xl text-center md:text-left">
          <h1 className="font-display font-semibold tracking-tight leading-[1.14] sm:leading-[1.1]
                         text-[clamp(2rem,6vw,4.8rem)] text-white anim-reveal">
          ...
          <p className="mt-4 sm:mt-7 text-[clamp(0.95rem,2vw,1.2rem)] leading-relaxed
      ...
      <section className="relative z-10 dot-grid">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16 sm:py-24 md:py-32">
          ...
          <div className="max-w-lg mb-10 sm:mb-14 text-center sm:text-left mx-auto sm:mx-0">
            ...
            <h2 className="font-display text-[clamp(1.85rem,5vw,3.5rem)] font-semibold
                           tracking-tight leading-[1.08] sm:leading-[1.05] text-white">
              Everything in<br className="hidden sm:block" /> one workspace
            </h2>
            <p className="mt-3 sm:mt-4 text-slate-400 sm:text-slate-500 leading-relaxed text-sm sm:text-[15px]">
          ...
          <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            {tools.map((t) => (
              <div key={t.name} className="tool-card rounded-xl sm:rounded-2xl p-4 sm:p-5 cursor-default group flex xs:flex-col items-start gap-3.5 xs:gap-0">
                <div className="w-10 h-10 rounded-xl bg-blue-400/[.08] border border-blue-400/[.12]
                                flex items-center justify-center shrink-0 xs:mb-4
                                group-hover:bg-blue-400/[.12] group-hover:border-blue-400/[.2]
                                transition-colors">
                  <t.icon size={18} className="text-blue-400" strokeWidth={1.8} />
                </div>
                <div>
                  <p className="text-[15px] font-semibold text-white tracking-tight mb-0.5 sm:mb-1">
                    {t.name}
                  </p>
                  <p className="text-[13px] text-slate-400 sm:text-slate-500 leading-snug">
                    {t.desc}
                  </p>
                </div>
              </div>
      ...
      <section className="relative z-10 border-t border-white/[.04]">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-16 sm:py-24 md:py-32">
          ...
          <div className="flex flex-col gap-0 max-w-md mx-auto sm:max-w-none">
            {steps.map((s, i) => (
              <div key={s.num}>
                {/* Step row */}
                <div className="flex items-start gap-4 sm:gap-5">
                  ...
                  <div className="pt-0.5 sm:pt-1">
                    <p className="text-xs font-medium text-brand-400/70 sm:text-brand-400/60 tracking-wider mb-1">
                      Step {s.num}
                    </p>
                    <h3 className="text-base font-semibold text-white tracking-tight mb-1">
                      {s.title}
                    </h3>
                    <p className="text-sm text-slate-400 sm:text-slate-500 leading-relaxed max-w-sm">
                      {s.body}
                    </p>
                  </div>
                </div>
                ...
                {i < steps.length - 1 && (
                  <div className="flex items-stretch gap-4 sm:gap-5 py-1">
                    <div className="w-11 sm:w-12 flex justify-center shrink-0">
      ...
      <section className="relative z-10 border-t border-white/[.04]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-20 sm:py-28 md:py-36 text-center">
          ...
          <p className="text-xs sm:text-sm text-slate-400 sm:text-slate-500 mb-3 sm:mb-4 relative">
            Free to use. No credit card required.
          </p>
          <h2 className="font-display text-[clamp(1.9rem,5.5vw,4.5rem)] font-semibold
                         tracking-tight leading-[1.05] sm:leading-[.95] text-white mb-8 sm:mb-10 relative">
            Start working with<br />
            your PDFs today.
          </h2>
          <div className="relative flex flex-col sm:flex-row justify-center items-center gap-3 w-full max-w-xs sm:max-w-none mx-auto">
            <Link to="/register"
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 sm:px-7 py-3.5 rounded-xl text-[15px] font-medium
                         text-white bg-gradient-to-b from-brand-500 to-brand-600
                         shadow-[0_2px_24px_rgba(45,98,255,.35)]
                         hover:shadow-[0_4px_32px_rgba(45,98,255,.5)]
                         hover:-translate-y-px active:translate-y-0 transition-all">
            ...
            <Link to="/login"
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 sm:px-7 py-3.5 rounded-xl text-[15px] font-medium
                         text-slate-300 sm:text-slate-400 hover:text-white border border-white/10 sm:border-transparent hover:border-white/20
                         hover:bg-white/[.03] transition-all">
              or sign in
            </Link>
          </div>
      ...
      <footer className="relative z-10 border-t border-white/[.04]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8 flex flex-col sm:flex-row items-center justify-between text-center sm:text-left gap-3 sm:gap-4">
          ...
          <p className="text-xs text-slate-500 sm:text-slate-600">
