  ...
  return (
    <div className="min-h-screen bg-[#050e18] text-slate-200 relative">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden" aria-hidden>
        <div className="absolute -top-[20%] left-1/2 -translate-x-1/2 w-[min(900px,120vw)] h-[min(900px,120vw)] rounded-full"
          style={{ background: 'radial-gradient(circle, rgba(45,98,255,.1) 0%, transparent 65%)' }} />
        <div className="absolute top-[55%] -right-[10%] w-[min(500px,60vw)] h-[min(500px,60vw)] rounded-full"
      ...
      <nav className="sticky top-0 z-50 backdrop-blur-xl border-b border-white/[.04]"
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between px-4 sm:px-6 h-14 sm:h-16">
          ...
          <button
            className="sm:hidden p-2 -mr-1 text-slate-400 hover:text-white transition-colors"
        ...
        {mobileMenuOpen && (
          <div className="sm:hidden border-t border-white/[.04] bg-[#050e18]/95 backdrop-blur-xl">
            <div className="px-4 py-3 flex flex-col gap-1">
              <Link to="/login"
                onClick={() => setMobileMenuOpen(false)}
                className="text-sm text-slate-400 hover:text-white px-3 py-2.5 rounded-lg transition-colors">
                Sign in
              </Link>
              <Link to="/register"
                onClick={() => setMobileMenuOpen(false)}
            ...
            {steps.map((s, i) => (
              <div key={s.num}>
                ...
                <div className="flex items-start gap-3 sm:gap-5">
                  ...
                  {/* Right: text */}
                  <div className="pt-1">
                    <p className="text-[11px] sm:text-xs font-medium text-brand-400/60 tracking-wider mb-1">
                      Step {s.num}
                    </p>
                ...
                {i < steps.length - 1 && (
                  <div className="flex items-stretch gap-3 sm:gap-5 py-1">
      ...
      {/* CTA */}
      <section className="relative z-10 border-t border-white/[.04]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16 sm:py-24 md:py-36 text-center">
          {/* Soft top glow */}
          ...
          <h2 className="font-display text-[clamp(1.8rem,5.5vw,4.5rem)] font-semibold
                         tracking-tight leading-[.98] text-white mb-8 sm:mb-10 relative">
            Start working with<br className="hidden sm:block" />
            your PDFs today.
          </h2>
          <div className="relative flex flex-col sm:flex-row flex-wrap justify-center gap-3 items-center">
            <Link to="/register"
      ...
      <footer className="relative z-10 border-t border-white/[.04]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8 flex flex-col sm:flex-row items-center sm:justify-between gap-2 sm:gap-4">
          <span className="font-display text-sm font-semibold text-slate-600">
            Crystal<span className="text-blue-400/50">PDF</span>
          </span>
          <p className="text-[11px] sm:text-xs text-slate-700 text-center">
