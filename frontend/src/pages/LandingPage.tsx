...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-[#050e18] text-slate-200 relative safe-x safe-bottom">
      {/* ── CSS ─────────────────────────────────────────────── */}
      <style>{`
      ...
      {/* ── Nav ────────────────────────────────────────────── */}
      <nav className="sticky top-0 z-50 backdrop-blur-xl border-b border-white/[.04] safe-top"
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between px-4 sm:px-6 h-16">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-3">
            <Link to="/login"
              className="text-sm font-medium text-slate-300 hover:text-white px-4 py-3 rounded-lg transition-colors w-full sm:w-auto text-center">
              Sign in
            </Link>
            <Link to="/register"
              className="text-sm font-medium text-blue-300 bg-brand-500/15 hover:bg-brand-500/25
                         ...
                         px-4 py-3 rounded-lg transition-all w-full sm:w-auto text-center">
              Get started
            </Link>
          </div>
        </div>
      ...
      <section className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6
                          pt-16 pb-20 sm:pt-24 sm:pb-28 lg:pt-32 lg:pb-36
                          flex flex-col lg:grid lg:grid-cols-[1fr,auto] items-center gap-8 lg:gap-12">
        {/* Crystal visual - shown on mobile ABOVE text, on desktop to the right */}
        <div className="lg:hidden w-full max-w-xs mx-auto anim-fade anim-d4">
          <div className="aspect-square max-w-[280px] mx-auto">
            <div className="prism w-full h-full">
        ...
        {/* Text column */}
        <div className="max-w-2xl w-full text-center lg:text-left lg:order-1">
          ...
          <p className="mt-5 sm:mt-6 text-[clamp(0.95rem,2vw,1.1rem)] leading-relaxed
                        text-slate-400 max-w-md mx-auto lg:mx-0 anim-reveal anim-d2">
            Merge, split, compress, protect, convert, and annotate—
            from one elegant workspace. No subscriptions, no upload limits.
          </p>

          <div className="mt-8 sm:mt-10 flex flex-col sm:flex-row gap-3 justify-center lg:justify-start anim-reveal anim-d3">
            <Link to="/register"
              className="inline-flex items-center justify-center gap-2 px-6 py-4 rounded-xl text-[15px] sm:text-[16px] font-medium
        ...
        <div className="hidden lg:flex items-center justify-center anim-fade anim-d4 lg:order-2">
          <div className="prism w-[340px] h-[340px]">
            <div className="absolute inset-0 flex items-center justify-center">
              <Layers size={48} className="text-blue-400/30" strokeWidth={1} />
            </div>
          </div>
      ...
      {/* ── Tools grid ─────────────────────────────────────── */}
      <section className="relative z-10 dot-grid safe-x">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-20 sm:py-24 lg:py-32">
          ...
          <div className="max-w-lg mb-10 sm:mb-14">
            ...
            <h2 className="font-display text-[clamp(1.875rem,5vw,3rem)] sm:text-[clamp(2rem,4.5vw,3.5rem)] font-semibold
                           tracking-tight leading-[1.05] text-white">
              Everything in<br className="hidden sm:block" /> one workspace
            </h2>
            <p className="mt-3 sm:mt-4 text-slate-500 leading-relaxed text-[14px] sm:text-[15px]">
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4">
            {tools.map((t) => (
              <div key={t.name} className="tool-card rounded-2xl p-4 sm:p-5 cursor-default group">
                ...
                <p className="text-[14px] sm:text-[15px] font-semibold text-white tracking-tight mb-1">
                  {t.name}
                </p>
      ...
      {/* ── How it works ───────────────────────────────────── */}
      <section className="relative z-10 border-t border-white/[.04] safe-x">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-20 sm:py-24 lg:py-32">
          <div className="text-center max-w-lg mx-auto mb-12 sm:mb-16">
            <p className="text-xs font-medium tracking-[.15em] uppercase text-brand-400 mb-3">
              How it works
            </p>
            <h2 className="font-display text-[clamp(1.75rem,5vw,2.75rem)] font-semibold
                           tracking-tight leading-[1.05] text-white">
          ...
          <div className="flex flex-col gap-0">
            {steps.map((s, i) => (
              <div key={s.num}>
                {/* Step row */}
                <div className="flex items-start gap-4 sm:gap-5 px-2 sm:px-0">
                  {/* Left: number + icon */}
                  ...
                  {/* Right: text */}
                  <div className="pt-1 w-full sm:max-w-sm">
                ...
                {i < steps.length - 1 && (
                  <div className="flex items-stretch gap-4 sm:gap-5 py-1 px-2 sm:px-0">
      ...
      {/* ── CTA ────────────────────────────────────────────── */}
      <section className="relative z-10 border-t border-white/[.04] safe-x">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-24 sm:py-28 lg:py-36 text-center">
          {/* Soft top glow */}
      ...
      {/* ── Footer ─────────────────────────────────────────── */}
      <footer className="relative z-10 border-t border-white/[.04] safe-x safe-bottom">
