    ...
    <div className="min-h-screen bg-[#050e18] text-slate-200 relative">
      ...
      <nav className="sticky top-0 z-50 backdrop-blur-xl border-b border-white/[.04]"
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between px-4 md:px-6 h-14 md:h-16">
          <Link to="/" className="flex items-center gap-2 group">
            <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-gradient-to-br from-brand-500 to-blue-400 flex items-center justify-center
                            shadow-lg shadow-brand-500/20 group-hover:shadow-brand-500/40 transition-shadow">
              <Gem size={12} className="text-white md:hidden" />
              <Gem size={14} className="text-white hidden md:block" />
            </div>
          ...
          <div className="flex items-center gap-1 md:gap-2">
            <Link to="/login"
              className="text-xs md:text-sm text-slate-400 hover:text-white px-2.5 md:px-4 py-2 transition-colors">
              Sign in
            </Link>
            <Link to="/register"
              className="text-xs md:text-sm font-medium text-blue-300 bg-brand-500/15 hover:bg-brand-500/25
                         border border-brand-500/25 hover:border-brand-500/40
                         px-3 md:px-4 py-2 rounded-lg transition-all whitespace-nowrap">
              Get started
            </Link>
          </div>
        </div>
      ...
      <section className="relative z-10 max-w-6xl mx-auto px-4 md:px-6
        ...
        <div className="max-w-2xl text-center md:text-left">
          <h1 className="font-display font-semibold tracking-tight leading-[1.1]
            ...
            Every PDF tool<br className="sm:hidden" /> you'll ever{' '}
            <em className="not-italic text-blue-400">need.</em>
          </h1>

          <p className="mt-5 md:mt-7 text-[clamp(0.9rem,1.8vw,1.2rem)] leading-relaxed
          ...
          <div className="mt-8 md:mt-10 flex flex-col sm:flex-row justify-center md:justify-start gap-3 anim-reveal anim-d3">
            <Link to="/register"
              className="inline-flex items-center justify-center gap-2 px-5 md:px-6 py-3 md:py-3.5 rounded-xl text-sm md:text-[15px] font-medium
              ...
              Start for free <ArrowRight size={14} strokeWidth={2.2} />
            </Link>
            <Link to="/login"
              className="inline-flex items-center justify-center gap-2 px-5 md:px-6 py-3 md:py-3.5 rounded-xl text-sm md:text-[15px] font-medium
        ...
        {/* Crystal visual - visible on mobile as a smaller decorative element */}
        <div className="flex md:hidden items-center justify-center anim-fade anim-d4 mt-4">
          <div className="prism">
      ...
      <section className="relative z-10 dot-grid">
        <div className="max-w-6xl mx-auto px-4 md:px-6 py-16 md:py-32">
          {/* Section header */}
          <div className="max-w-lg mb-10 md:mb-14 text-center md:text-left">
            <p className="text-xs font-medium tracking-[.15em] uppercase text-brand-400 mb-3 md:mb-4">
              Toolkit
            </p>
            ...
            <p className="mt-3 md:mt-4 text-slate-500 leading-relaxed text-sm md:text-[15px]">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 md:gap-3">
            {tools.map((t) => (
              <div key={t.name} className="tool-card rounded-xl md:rounded-2xl p-3.5 md:p-5 cursor-default group">
                <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl bg-blue-400/[.08] border border-blue-400/[.12]
                                flex items-center justify-center mb-3 md:mb-4
                                group-hover:bg-blue-400/[.12] group-hover:border-blue-400/[.2]
                                transition-colors">
                  <t.icon size={14} className="text-blue-400 md:hidden" strokeWidth={1.8} />
                  <t.icon size={18} className="text-blue-400 hidden md:block" strokeWidth={1.8} />
                </div>
                <p className="text-sm md:text-[15px] font-semibold text-white tracking-tight mb-0.5 md:mb-1">
                  {t.name}
                </p>
                <p className="text-xs md:text-[13px] text-slate-500 leading-snug">
                  {t.desc}
                </p>
              </div>
            ))}
      ...
      <section className="relative z-10 border-t border-white/[.04]">
        <div className="max-w-3xl mx-auto px-4 md:px-6 py-16 md:py-32">
          ...
          <div className="flex flex-col gap-0">
            {steps.map((s, i) => (
              <div key={s.num}>
                ...
                <div className="flex items-start gap-4 md:gap-5">
                  ...
                  <div className="flex flex-col items-center">
                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-lg md:rounded-xl flex items-center justify-center
                      ...
                      <s.icon size={16} className="text-blue-400 md:hidden" strokeWidth={1.6} />
                      <s.icon size={20} className="text-blue-400 hidden md:block" strokeWidth={1.6} />
                    </div>
                  </div>

                  {/* Right: text */}
                  <div className="pt-0.5 md:pt-1">
                    <p className="text-[10px] md:text-xs font-medium text-brand-400/60 tracking-wider mb-0.5 md:mb-1">
                      Step {s.num}
                    </p>
                    <h3 className="text-sm md:text-base font-semibold text-white tracking-tight mb-1 md:mb-1.5">
                      {s.title}
                    </h3>
                ...
                {i < steps.length - 1 && (
                  <div className="flex items-stretch gap-4 md:gap-5 py-1">
                    <div className="w-10 md:w-12 flex justify-center shrink-0">
                      <div className="w-px h-6 md:h-8 bg-gradient-to-b from-brand-500/20 to-transparent" />
                    </div>
                  </div>
                )}
              </div>
      ...
      {/* ── CTA ────────────────────────────────────────────── */}
      <section className="relative z-10 border-t border-white/[.04]">
        <div className="max-w-6xl mx-auto px-4 md:px-6 py-20 md:py-36 text-center">
          {/* Soft top glow */}
          ...
          <p className="text-xs md:text-sm text-slate-500 mb-3 md:mb-4 relative">
            Free to use. No credit card required.
          </p>
          <h2 className="font-display text-[clamp(1.8rem,5.5vw,4.5rem)] font-semibold
                         tracking-tight leading-[.95] text-white mb-8 md:mb-10 relative">
            Start working with<br />
            your PDFs today.
          </h2>
          <div className="relative flex flex-col sm:flex-row justify-center gap-3">
            <Link to="/register"
              className="inline-flex items-center justify-center gap-2 px-5 md:px-7 py-3 md:py-3.5 rounded-xl text-sm md:text-[15px] font-medium
                         text-white bg-gradient-to-b from-brand-500 to-brand-600
                         shadow-[0_2px_24px_rgba(45,98,255,.35)]
                         hover:shadow-[0_4px_32px_rgba(45,98,255,.5)]
                         hover:-translate-y-px transition-all">
              Create free account <ArrowRight size={14} strokeWidth={2.2} />
            </Link>
            <Link to="/login"
              className="inline-flex items-center justify-center gap-2 px-5 md:px-7 py-3 md:py-3.5 rounded-xl text-sm md:text-[15px] font-medium
      ...
      <footer className="relative z-10 border-t border-white/[.04]">
        <div className="max-w-6xl mx-auto px-4 md:px-6 py-6 md:py-8 flex flex-col md:flex-row items-center justify-between gap-3 md:gap-4">
          <span className="font-display text-sm font-semibold text-slate-600">
            Crystal<span className="text-blue-400/50">PDF</span>
          </span>
          <p className="text-[10px] md:text-xs text-slate-700 text-center md:text-left">
